...
class IPLS(torch.jit.ScriptModule):
    """
    ...
    with burn-in steps, prior on means, inner loop, and multivariate targets
    """
    def __init__(self,
        n_feat:int,
        n_target:int,
        ...
        # squared norm of tz (unnormalized scores)
    ...
    def partial_fit(self, x, y):
        ...
        # burn-in steps: update means and 
...
def fit(X, Y, n_latent=8, burn_in=2, inner_steps=2):
    """create in IPLS model, fit to data, print timing and return the predictor"""
    n_feat = X.shape[-1]
    n_target = Y.shape[-1]

    ipls = IPLS(n_feat, n_target, n_latent,
        burn_in=burn_in, inner_steps=inner_steps, n_latent=n_latent)
    ...
    print(f'partial_fit time (mean): {np.mean(ts)*1e-6} ms')
    print(f'partial_fit time (99%): {np.quantile(ts, 0.99)*1e-6} ms')
